    ...
    if(memberobj.id) {
      if(server.memberMap[memberobj.id].roles.length) {
        if(memberobj.username != "DNBOT") {
          verified.push({"username": memberobj.username.toLowerCase(), "id": memberobj.id});
        }
      }
    }
  }
...
//username filtering
  ...
  var usercheck = getcache.find({"username": msg.sender.username.toLowerCase()});
  if(usercheck.length) {
...
//global message logging
  if(msg.sender.username != "DNBOT" & msg.sender.username != "BotVentic") {
    var date = new Date();
    var logobject = {sender: msg.sender.username, senderid: msg.sender.id, message: msg.content, messageid: msg.id, date: date, channel: msg.channel.name, channelid: msg.channel.id};
